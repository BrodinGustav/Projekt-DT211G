    ...
    try {
        ...
        if (layer instanceof L.Marker) {
          map.removeLayer(layer);
            ...
            marker.bindPopup("<b>Du \xe4r h\xe4r</b>", {}).openPopup();
...
function displayCityInfo(nominatimData) {
    ...
    if (nominatimData.address) {
        ...
        const cityInfo = `
      <b>Stad:</b> ${city || "Ej tillg\xe4nglig"} <br>
      
      <b>Land:</b> ${country || "Ej tillg\xe4nglig"}
    `;
        //Utskrift DOM
        cityInfoContainer.innerHTML = `<p style="font-weight: bold;"></p> ${cityInfo}`;
    } else {
        console.error("Stadsinformation ej tillg\xe4nglig.");
...
/***************************************************************************************/ async function getWeatherInfo(firstResult) {
    ...
    try {
        ...
        if (weatherData.main) {
            ...
            const weatherInfo = `
        <b>Temperatur:</b> ${temperatureCelsius} \xb0C <br> 
        <b>Upplevd temperatur:</b> ${feelsLikeTemperatureCelsius} \xb0C <br> 
        <b>Vindhastighet:</b> ${windSpeed} m/s <br> 
        <b>V\xe4derbeskrivning:</b> ${weatherDescription || "Ej tillg\xe4nglig"}
        <img src="https://openweathermap.org/img/wn/${weatherIcon}.png" alt="V\xe4derikon">
      `;
            if (temperatureCelsius < 0) weatherContainer.style.backgroundColor = "blue";
            else if (temperatureCelsius >= 0 && temperatureCelsius <= 10) weatherContainer.style.backgroundColor = "lightblue"; // Du kan använda en lämplig färg här
            else weatherContainer.style.backgroundColor = "orangered";
            //Utskrift DOM
            weatherContainer.innerHTML = `<p style="font-weight: bold;"></p> ${weatherInfo}`;
        } else {
            console.error("V\xe4derinformation ej tillg\xe4nglig.");
